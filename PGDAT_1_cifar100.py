...
save_folder = os.path.join('./PGDAT', 'cifar100', model_str, '%s_%s_%s_%s' % (attack_str, opt_str, decay_str, loss_str))
print(save_folder)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        if args.Lambda != 0:
            loss = loss + F.cross_entropy(logits_adv, labels)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    if eval_this_epoch:
        ...
        val_str = 'Epoch %d | Validation | Time: %.4f | lr: %s | SA: %.4f, RA1: %.4f, RA2: %.4f, linf: %.4f - %.4f' % (
