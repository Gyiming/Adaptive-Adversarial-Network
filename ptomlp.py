...
def main():
    ...
    normal_vector_vali = vectorprocess('/u/ygan10/Once-for-All-Adversarial-Training/metrics/c100_tpgd_ori_test.csv')
    adv_vector_vali = vectorprocess('/u/ygan10/Once-for-All-Adversarial-Training/metrics/c100_tpgd8_test.csv')
    

    train_vector = np.concatenate((normal_vector_train,adv_vector_train),axis = 0)
    vali_vector = np.concatenate((normal_vector_vali, adv_vector_vali), axis = 0)
    ...
    train_label = np.concatenate((normal_label_train, adv_label_train), axis = 0)
    vali_label = np.concatenate((normal_label_vali, adv_label_vali), axis = 0)
    ...
    torch.save(model.state_dict(), os.path.join(save_folder, 'best_c100.pth'))
