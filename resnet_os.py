...
class ResNet34OAT(nn.Module):
    ...
    def __init__(self, num_classes=10, FiLM_in_channels=1, use2BN=False):
        ...
        self.detect1 = nn.Conv2d(384,64,kernel_size = 3, stride = 1, padding = 1, bias = False)
        self.detect2 = nn.Conv2d(64,32,kernel_size = 3, stride = 1, padding = 1, bias = False)
        self.detect3 = nn.Conv2d(32,16,kernel_size = 3, stride = 1, padding = 1, bias = False)
        self.detect_l1 = nn.Linear(64,32)
        self.detect_l2 = nn.Linear(32,2)
    ...
    def forward(self, x, _lambda, idx2BN=None):
        ...
        d_ = self.detect1(detector_input)
        d_ = F.relu(d_)
        ...
        for bundle in self.bundles[1:]:
            for block in bundle:
                out_normal = block(out_normal, _lambda, 0)
                out_adv = block(out_adv, _lambda, 1)
        #pdb.set_trace()
