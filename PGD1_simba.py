...
def main():
    ...
    #simba args:
    parser.add_argument('--num_iters', type=int, default=8000, help='maximum number of iterations, 0 for unlimited')
    parser.add_argument('--log_every', type=int, default=10, help='log every n iterations')
    parser.add_argument('--epsilon', type=float, default=0.4, help='step size per iteration')
    parser.add_argument('--linf_bound', type=float, default=1.0, help='L_inf bound for frequency space attack')
    ...
    for i, (imgs, labels) in enumerate(val_loader):
        ...
        adv, probs, succs, queries, l2_norms, linf_norms = attacker.simba_batch(imgs, labels, max_iters, args.freq_dims, args.stride, args.epsilon, linf_bound=args.linf_bound, order=args.order, targeted=args.targeted, pixel_attack=args.pixel_attack, log_every=args.log_every)
            
        imgs, adv_imgs, labels = imgs.cuda(), adv.cuda(), labels.cuda()

        logits_c = model(imgs)
        logits_a = model(adv_imgs)
        #pdb.set_trace()
        val_accs.append((logits_c.argmax(1) == labels).float().mean().item())
        val_accs_adv1.append((logits_a.argmax(1) == labels).float().mean().item())
        pdb.set_trace()  
        print('evaluate one batch')
