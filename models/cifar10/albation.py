...
class ResNet34OAT(nn.Module):
    ...
    def __init__(self, num_classes=10, FiLM_in_channels=1, use2BN=False):
        ...
        self.bundle1a = nn.ModuleList([
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
        ])
        self.bundle1c = nn.ModuleList([
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
            BasicBlockOAT(64, 64, 64, stride=1, use2BN=use2BN, FiLM_in_channels=FiLM_in_channels),
        ])
        
        self.bundle2a = nn.ModuleList([
        ...
        self.bundlesa = [self.bundle1a, self.bundle2a, self.bundle3a, self.bundle4a]
        self.bundlesc = [self.bundle1c, self.bundle2c, self.bundle3c, self.bundle4c]
    ...
    def forward(self, x, _lambda, idx2BN=None):
        ...
        if idx2BN == 0:
            ...
            out = F.conv2d(x, weight = out[0:64,0:3,:,:], bias=None, stride = [1,1], padding = [0,0], dilation = [1,1],groups = 1)
        elif idx2BN == x.size()[0]:
            out = self.convf1c(x)
            m = nn.AdaptiveAvgPool2d((3,3))
            out = m(out)
            #print(out[:,0:3,:,:].size())
        ...
        #out = self.film1(out, _lambda)
        out = F.relu(out)
        for i in range(len(self.bundlesa)):
            if idx2BN == 0:
                for block in self.bundlesa[i]:
                    out = block(out, _lambda, idx2BN)
            elif idx2BN == x.size()[0]:
                for block in self.bundlesc[i]:
                    out = block(out, _lambda, idx2BN)
...
if __name__ == '__main__':
    ...
    net = ResNet34OAT(num_classes=100)
