...
from dynamic_models.dy_resnet import resnet34 as dy_resnet34
from dynamic_models.dy_resnet_attloss import r34dyn1 as dy1_resnet34
...
parser.add_argument('--epochs', '-e', default=350, type=int, help='number of total epochs to run')
...
parser.add_argument('--lr', default=0.12, type=float, help='initial learning rate')
...
if args.dataset == 'cifar10':
    ...
    model_fn = WRN40_2OAT
#pdb.set_trace()
model = dy1_resnet34(num_classes=10).cuda()
#model = torch.nn.DataParallel(model)
pdb.set_trace()
aaa = torch.randn(4,3,32,32).cuda()
bbb,att = model(aaa)
...
save_folder = os.path.join('./sample_DYN', 'cifar10', model_str, '%s_%s_%s_%s' % (attack_str, opt_str, decay_str, lambda_str))
print(save_folder)
...
#pdb.set_trace()
if args.resume:
    last_epoch, best_TA, best_ATA1, training_loss, val_TA, val_ATA1 \
         = load_ckpt(model, optimizer, scheduler, os.path.join(save_folder, 'latest.pth'))
    start_epoch = last_epoch + 1
    f = open(save_folder+'/val_log.txt','r')
    a = f.readlines()
    real_val_tpgd = []
    for i in range(len(a)):
        real_val_tpgd.append(float(a[i][-7:-1]))
    val_ATA2 = []
    t_ = 0
    best_ATA2 = 0
    for i in range(start_epoch):
        if (i % 10 == 0) or (i>=int(0.7*args.epochs)):
            val_ATA2.append(real_val_tpgd[t_])
            if real_val_tpgd[t_] > best_ATA2:
                best_ATA2 = real_val_tpgd[t_]
            t_ += 1
        else:
            val_ATA2.append(val_ATA2[i-1])
    pdb.set_trace()
else:
    start_epoch = 0
...
# attacker:
attacker = PGD(eps=args.eps/255, steps = args.steps, use_FiLM=False)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        with ctx_noparamgrad_and_eval(model):
            imgs_adv1 = attacker.attack(model,imgs,labels = labels)
            #imgs_adv2 = attacker2.attack(model, imgs, labels=labels, _lambda=vector_att2.repeat(imgs.size()[0],1,1), idx2BN=0)
        logits_adv1,att1 = model(imgs_adv1.detach())
        #logits_adv2 = model(imgs_adv2.detach(), vector_att2.repeat(imgs_adv2.size()[0],1,1), idx2BN = 0)
        # logits for clean imgs:
        logits,att2 = model(imgs)
        label_att_normal = torch.ones(att1.size()[0],att1.size()[1])
        label_att_adv = torch.zeros(att1.size()[0],att1.size()[1])
