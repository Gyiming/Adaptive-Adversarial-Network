...
model = model_fn(use2BN=False, FiLM_in_channels=FiLM_in_channels).cuda()
model = torch.nn.DataParallel(model)

# mkdirs:
save_folder = '/u/ygan10/Once-for-All-Adversarial-Training/2BNDYN/cifar10/ResNet34OAT-2BN/onlyl1/'
print(save_folder)
...
label_adv = np.load('./testimg/adv_single_pgd2.npy')
label_adv_gt = np.load('./testimg/adv_single.npy')
...
for i, (imgs, labels, adv_labels, adv_labels_gt) in enumerate(valiloader):
    ...
    if len(index_adv1) == 0 or len(index_normal) == 0:
        ...
        #pdb.set_trace()
        for ttt in range(len(index_adv1)):
            if logits_adv1.argmax(1)[ttt] == labels_adv1[ttt]:
                if index_adv1[ttt] in index_normal_gt:
                    correct_normal += 1
                elif index_adv1[ttt] in index_adv1_gt:
                    correct_adv += 1
        #pdb.set_trace()
        ...
        for ttt in range(len(index_normal)):
            if logits.argmax(1)[ttt] == labels_normal[ttt]:
                if index_normal[ttt] in index_normal_gt:
                    correct_normal += 1
                    fuck += 1
                elif index_normal[ttt] in index_adv1_gt:
                    correct_adv += 1
        #pdb.set_trace()
        #total_normal += len(index_normal)
        #total_adv += len(index_adv1)
...
print(val_str)
#pdb.set_trace()
print(correct_normal/5000)
print(correct_adv/5000)
val_str = 'SA: %.4f, RAtpgd: %.4f' % (val_accs.avg, val_accs_adv2.avg)   
print(val_str)
