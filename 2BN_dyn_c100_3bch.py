...
from models.cifar10.resnet_dynf_3 import ResNet34OAT
...
model = model_fn(num_classes = 100, use2BN = True, FiLM_in_channels=FiLM_in_channels).cuda()
...
save_folder = os.path.join('./2BNDYN', 'cifar100', model_str, '%s_%s_%s_%s' % (attack_str, opt_str, decay_str, lambda_str))
print(save_folder)
...
attacker2 = PGD(eps=(args.eps+6)/255, steps = args.steps, use_FiLM = True)
...
for epoch in range(start_epoch, args.epochs):
    ...
    for i, (imgs, labels) in enumerate(train_loader):
        ...
        loss = F.cross_entropy(logits, labels) + F.cross_entropy(logits_adv1, labels) + F.cross_entropy(logits_adv2, labels)
