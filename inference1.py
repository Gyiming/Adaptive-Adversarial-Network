...
# mkdirs:
save_folder = '/localdisk2/OATModel/2attack/8_2.1advloss/'
print(save_folder)
...
#pdb.set_trace()
#last_epoch, best_TA, best_ATA, training_loss, val_TA, val_ATA = load_ckpt(model, optimizer, scheduler, os.path.join(save_folder, 'latest.pth'))
...
vector_att1 = torch.from_numpy(np.array([8/255])).unsqueeze(0).float().cuda()
...
for i, (imgs, labels, adv_labels,adv_labels_gt) in enumerate(valiloader):
    ...
    if len(index_adv1) == 0 or len(index_normal) == 0:
        ...
        if len(index_adv1) == 0:
            ...
            logits_all = model(imgs_normal,vector_normal.repeat(imgs_normal.size()[0],1,1), idx2BN = imgs_normal.size()[0])
            normal_gt = []
            adv_gt = []
            for t in range(len(adv_labels_gt)):
                if adv_labels_gt[t] == 0:
                    ...
                    adv_gt.append(t)
            #pdb.set_trace()
            val_accs_adv1.append((logits_all[adv_gt].argmax(1) == labels[adv_gt]).float().mean().item())
            val_accs.append((logits_all[normal_gt].argmax(1) == labels[normal_gt]).float().mean().item())
        else:
            #only use adv branch
            logits_all = model(imgs_adv1.detach(), vector_att1.repeat(imgs_adv1.size()[0],1,1), idx2BN = 0)
            normal_gt = []
            adv_gt = []
            for t in range(len(adv_labels_gt)):
                if adv_labels_gt[t] == 0:
...
for i, (imgs, labels) in enumerate(val_loader):
    ...
    for imagesave_idx in range(len(labels)):
        pkl_save_dir = '/u/ygan10/Once-for-All-Adversarial-Training/ori_vali_1/' + str(labels[imagesave_idx].cpu().numpy()) + '/' + str(img_num[labels[imagesave_idx]]) + '.pkl'
        with gzip.open(pkl_save_dir,'wb',compresslevel=6) as f:
            pkl.dump(imgs[imagesave_idx].cpu().numpy(),f)
        pkl_save_dir = '/u/ygan10/Once-for-All-Adversarial-Training/ori_vali_1_ptolemy/' + str(labels[imagesave_idx].cpu().numpy()) + '/' + str(img_num[labels[imagesave_idx]]) + '.pkl'
        with gzip.open(pkl_save_dir,'wb',compresslevel=6) as f:
    ...
    for imagesave_idx in range(len(labels)):
        pkl_save_dir = '/u/ygan10/Once-for-All-Adversarial-Training/pgd8_vali_1/' + str(labels[imagesave_idx].cpu().numpy()) + '/' + str(img_num_adv1[labels[imagesave_idx]]) + '.pkl'
        with gzip.open(pkl_save_dir,'wb',compresslevel=6) as f:
            pkl.dump(imgs_adv1[imagesave_idx].detach().cpu().numpy(),f)
        pkl_save_dir = '/u/ygan10/Once-for-All-Adversarial-Training/pgd8_vali_1_ptolemy/' + str(labels[imagesave_idx].cpu().numpy()) + '/' + str(img_num_adv1[labels[imagesave_idx]]) + '.pkl'
        with gzip.open(pkl_save_dir,'wb',compresslevel=6) as f:
...
val_str = 'SA: %.4f, RA1: %.4f' % (val_accs.avg, val_accs_adv1.avg)
print(val_str)