...
class SimBA:
    ...
    def __init__(self, model, dataset, image_size, vector):
        self.model = model
    ...
    def normalize(self, x):
        return simbautils.apply_normalization(x, self.dataset)

    def get_probs(self, x, y):
        output = self.model(self.normalize(x.cuda()),self.vector.repeat(x.size()[0],1,1), idx2BN = 0).cpu()
    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        if order == 'rand':
            ...
            indices = simbautils.diagonal_order(image_size, 3)[:max_iters]
        elif order == 'strided':
            indices = simbautils.block_order(image_size, 3, initial_size=freq_dims, stride=stride)[:max_iters]
        else:
            indices = simbautils.block_order(image_size, 3)[:max_iters]
        if order == 'rand':
        ...
        if pixel_attack:
            ...
            trans = lambda z: simbautils.block_idct(z, block_size=image_size, linf_bound=linf_bound)
